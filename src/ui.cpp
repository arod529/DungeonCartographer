...
, map{map}
{
  ...
  menu->signal_activate().connect(sigc::mem_fun(*this, &UI::insertColumns));
	...
	builder->get_widget("menu_deleteColumns", menu);
  menu->signal_activate().connect(sigc::mem_fun(*this, &UI::deleteColumns));
...
/*!
  Displays a delete columns dialog. Deletes the number of specified columns starting
  at a specified column number.
**/
void UI::deleteColumns()
{
	//create dialog
  auto deleteDialog = Gtk::Dialog("Delete Columns", *this, Gtk::DialogFlags::DIALOG_MODAL|Gtk::DialogFlags::DIALOG_DESTROY_WITH_PARENT);
  deleteDialog.add_button("Accept", 1)->grab_default();
  deleteDialog.add_button("Cancel", 0);

  auto dialogBuilder = Gtk::Builder::create_from_resource(uiInsertColumn);
  ...
  auto adjustment_colNum = (Gtk::Adjustment*)dialogBuilder->get_object("adjustment_colNum").get();
  adjustment_colNum->set_upper(map->getLevelWidth(currPage));

  if(deleteDialog.run())
  {
    auto adjustment_count = (Gtk::Adjustment*)dialogBuilder->get_object("adjustment_count").get();

    map->deleteColumns(currPage, (int)adjustment_colNum->get_value(), (int)adjustment_count->get_value());
...
{
	//create dialog
  auto insertDialog = Gtk::Dialog("Insert Columns", *this, Gtk::DialogFlags::DIALOG_MODAL|Gtk::DialogFlags::DIALOG_DESTROY_WITH_PARENT);
  ...
  if(insertDialog.run())
  {
    ...
    map->insertColumns(currPage, (int)adjustment_colNum->get_value(), (int)adjustment_count->get_value());
